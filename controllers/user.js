...
module.exports = {
    ...
    logout: async ctx => {
        ...
        await dataBash.User.userLogout({
            where: {
                userCode: authorization
            }
        })
    ...
    favoritesList: async ctx => {
        ...
        // console.log(search)
        search = await dataBash.House.houseFind({
            where: {
                [Op.or]: search
            }
        });
    ...
    addHouse: async ctx => {
        let houseData = {
            ...
            "houseImgs": ctx.request.body.houseImg,
        ...
        let data = {
            ...
            priceNum: houseData.price,
            size: houseData.size
        };
        // 轮播图
        if (houseData.houseImgs.length) {
            data.carouselMap = houseData.houseImgs;
        }
        // 获取用户的朝向
        if (houseData.oriented) {
            data.orientedID = houseData.oriented;
            // console.log(data.orientedID)
            data.orientedName = (await dataBash.House.orientedFindOne({
                where: {
                    code: houseData.oriented
                }
            })).name;
        }
        // 获取用户的标签
        if (houseData.supporting.length) {
            data.supporting = houseData.supporting;
        }
        // 获取 房屋类型
        if (houseData.roomType) {
            data.roomTypeID = houseData.roomType;
            data.roomTypeName = (await dataBash.House.roomTypeFindOne({
                where: {
                    code: houseData.roomType
                }
            })).name;
        }
        ...
        if (houseData.community) {
            ...
            data.community = cityName.communityName;

            data.coord = (await dataBash.House.houseFindOne({
                where: {
                    community: {
                        [Op.like]: `%${data.community}%`
                    }
                }
            }));
            // console.log(data.community , data.coord)
            if (data.coord) data.coord = data.coord.coord
            else data.coord = {"latitude": "36.086776", "longitude": "103.771724"};
    ...
    shelving: async ctx => {
        ...
        let fl = await dataBash.House.houseUpdate({
            where: {
                houseCode: id,
                userCode: userCode.ID
            }
